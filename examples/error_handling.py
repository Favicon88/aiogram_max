...
from aiogram_max import Bot, Dispatcher, types
from aiogram_max.client.default import DefaultBotProperties
from aiogram_max.enums import ParseMode
from aiogram_max.filters import (
    Command,
    CommandObject,
    ExceptionMessageFilter,
    ExceptionTypeFilter,
)
from aiogram_max.types import ErrorEvent
...
async def handle_invalid_age_exception(event: ErrorEvent, bot: Bot) -> None:
    ...
    logger.error(
        "Error caught: %r while processing %r", event.exception, event.update
    )
...
async def handle_set_age(
    ...
    if not age:
        raise InvalidAge(
            "No age provided. Please provide your age as a command argument."
        )
...
async def handle_set_name(
    ...
    if not name:
        raise InvalidName(
            "Invalid name. Please provide your name as a command argument."
        )
