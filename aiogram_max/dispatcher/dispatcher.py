...
class Dispatcher(Router):
    ...
    async def feed_update(
        ...
        if update.message.bot != bot:
            ...
            update = Update.model_validate(
                update.model_dump(), context={"bot": bot}
            )
    ...
    async def feed_raw_update(
        ...
        return await self._feed_webhook_update(
            bot=bot, update=parsed_update, **kwargs
        )
    ...
    async def silent_call_request(
        cls, bot: Bot, result: TelegramMethod[Any]
    ) -> None:
        """
        ...
        **kwargs: Any,
        ...
        def process_response(task: Future[Any]) -> None:
            ...
            if isinstance(result, TelegramMethod):
                asyncio.ensure_future(
                    self.silent_call_request(bot=bot, result=result)
                )
        ...
        try:
            ...
            if process_updates.done():
                ...
                process_updates.remove_done_callback(release_waiter)
                process_updates.add_done_callback(
                    process_response, context=ctx
                )
    ...
    async def start_polling(
        ...
        **kwargs: Any,
        ...
        if not bots:
            raise ValueError(
                "At least one bot instance is required to start polling"
            )
        if "bot" in kwargs:
            raise ValueError(
        ...
        async with (
            self._running_lock
        ):  # Prevent to run this method twice at a once
                ...
                try:
                    ...
                    if close_bot_session:
                        await asyncio.gather(
                            *(bot.session.close() for bot in bots)
                        )
