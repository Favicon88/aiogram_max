...
from aiogram_max.dispatcher.event.bases import (
    MiddlewareEventType,
    MiddlewareType,
    NextMiddlewareType,
)
from aiogram_max.dispatcher.event.handler import CallbackType
...
class MiddlewareManager(Sequence[MiddlewareType[TelegramObject]]):
    ...
    def __getitem__(
        self, item: slice
    ) -> Sequence[MiddlewareType[TelegramObject]]:
        pass
    ...
    def wrap_middlewares(
        ...
        def handler_wrapper(
            event: TelegramObject, kwargs: Dict[str, Any]
        ) -> Any:
