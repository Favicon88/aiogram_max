...
from aiogram_max.filters.base import Filter
from aiogram_max.types import BotCommand, Message
from aiogram_max.utils.deep_linking import decode_payload
...
class Command(Filter):
    ...
    def extract_command(self, text: str) -> CommandObject:
        ...
        prefix, (command, _, mention) = full_command[0], full_command[
            1:
        ].partition("@")
    ...
    def validate_command(self, command: CommandObject) -> CommandObject:
        for allowed_command in cast(
            Sequence[CommandPatternType], self.commands
        ):
