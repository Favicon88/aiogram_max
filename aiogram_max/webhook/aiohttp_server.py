...
from aiogram_max.webhook.security import IPFilter
...
def check_ip(ip_filter: IPFilter, request: web.Request) -> Tuple[str, bool]:
    ...
    if peer_name := cast(Transport, request.transport).get_extra_info(
        "peername"
    ):
        host, _ = peer_name
...
class BaseRequestHandler(ABC):
    ...
    async def _background_feed_update(
        self, bot: Bot, update: Dict[str, Any]
    ) -> None:
    ...
    async def _handle_request(
        ...
        return web.Response(
            body=self._build_response_writer(bot=bot, result=result)
        )
