...
from aiogram_max.utils.dataclass import dataclass_kwargs
...
def get_sorted_mro_attrs_resolver(
    cls: type,
) -> Generator[tuple[str, Any], None, None]:
    """
    ...
    for position_in_mro, base in enumerate(inspect.getmro(cls)):
        for position_in_class, (name, value) in enumerate(vars(base).items()):
            ...
            attributes[name] = _AttributeContainer(
