...
from aiogram_max.enums import MessageEntityType
from aiogram_max.types import MessageEntity, User
...
class Text(Iterable[NodeType]):
    ...
    def from_entities(cls, text: str, entities: List[MessageEntity]) -> "Text":
        return cls(
            *_unparse_entities(
                ...
                entities=(
                    sorted(entities, key=lambda item: item.offset)
                    if entities
                    else []
                ),
            )
    ...
    def _render_entity(self, *, offset: int, length: int) -> MessageEntity:
        ...
        ), "Node without type can't be rendered as entity"
...
class CustomEmoji(Text):
    ...
    def __init__(
        self, *body: NodeType, custom_emoji_id: str, **params: Any
    ) -> None:
...
def _apply_entity(entity: MessageEntity, *nodes: NodeType) -> NodeType:
    ...
    return node_type(
        *nodes,
        **entity.model_dump(
            exclude={"type", "offset", "length"}, warnings=False
        ),
    )
...
def _unparse_entities(
    ...
    for index, entity in enumerate(entities):
        ...
        sub_entities = list(
            filter(
                lambda e: e.offset * 2 < (offset or 0), entities[index + 1 :]
            )
        )
        yield _apply_entity(
            entity,
...
def as_numbered_list(
    *items: NodeType, start: int = 1, fmt: str = "{}. "
) -> Text:
    """
