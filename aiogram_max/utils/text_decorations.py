...
if TYPE_CHECKING:
    from aiogram_max.types import MessageEntity
...
class TextDecoration(ABC):
    def apply_entity(self, entity: MessageEntity, text: str) -> str:
        ...
        if entity.type == MessageEntityType.TEXT_MENTION:
            from aiogram_max.types import User
    ...
    def _unparse_entities(
        ...
        for index, entity in enumerate(entities):
            ...
            if entity.offset * 2 > offset:
                yield self.quote(
                    remove_surrogates(text[offset : entity.offset * 2])
                )
            ...
            sub_entities = list(
                filter(
                    lambda e: e.offset * 2 < (offset or 0),
                    entities[index + 1 :],
                )
            )
...
class MarkdownDecoration(TextDecoration):
    MARKDOWN_QUOTE_PATTERN: Pattern[str] = re.compile(
        r"([_*\[\]()~`>#+\-=|{}.!\\])"
    )
    ...
    def quote(self, value: str) -> str:
        return re.sub(
            pattern=self.MARKDOWN_QUOTE_PATTERN, repl=r"\\\1", string=value
        )
