...
from aiogram_max.dispatcher.flags import get_flag
from aiogram_max.exceptions import CallbackAnswerException
from aiogram_max.methods import AnswerCallbackQuery
from aiogram_max.types import CallbackQuery, TelegramObject
...
class CallbackAnswer:
    ...
    def disabled(self, value: bool) -> None:
        if self._answered:
            raise CallbackAnswerException(
                "Can't change disabled state after answer"
            )
    ...
    def show_alert(self, value: Optional[bool]) -> None:
        if self._answered:
            raise CallbackAnswerException(
                "Can't change show_alert after answer"
            )
    ...
    def cache_time(self, value: Optional[int]) -> None:
        if self._answered:
            raise CallbackAnswerException(
                "Can't change cache_time after answer"
            )
