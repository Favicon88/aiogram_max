...
from aiogram_max.dispatcher.middlewares.base import BaseMiddleware
from aiogram_max.dispatcher.middlewares.user_context import (
    EVENT_CONTEXT_KEY,
    EventContext,
)
from aiogram_max.fsm.context import FSMContext
from aiogram_max.fsm.storage.base import (
    DEFAULT_DESTINY,
    BaseEventIsolation,
    BaseStorage,
    StorageKey,
)
from aiogram_max.fsm.strategy import FSMStrategy, apply_strategy
from aiogram_max.types import TelegramObject
...
class FSMContextMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        if context:
            ...
            async with self.events_isolation.lock(key=context.key):
                data.update(
                    {"state": context, "raw_state": await context.get_state()}
                )
    ...
    def resolve_event_context(
        ...
        event_context: EventContext = cast(
            EventContext, data.get(EVENT_CONTEXT_KEY)
        )
