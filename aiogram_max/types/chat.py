...
#     accent_color_id: Optional[int] = Field(
...
# .. deprecated:: API:7.3
#    https://core.telegram.org/bots/api-changelog#may-6-2024"""
#     personal_chat: Optional[Chat] = Field(
#         None, json_schema_extra={"deprecated": True}
#     )
...
#         from aiogram_max.methods import BanChatSenderChat
...
#         from aiogram_max.methods import UnbanChatSenderChat
...
#         from aiogram_max.methods import GetChatAdministrators
...
#         from aiogram_max.methods import DeleteMessage
...
#         from aiogram_max.methods import RevokeChatInviteLink
...
#         from aiogram_max.methods import EditChatInviteLink
...
#         from aiogram_max.methods import CreateChatInviteLink
...
#         from aiogram_max.methods import ExportChatInviteLink
...
#         from aiogram_max.methods import SendChatAction
...
#         from aiogram_max.methods import DeleteChatStickerSet
...
#         from aiogram_max.methods import SetChatStickerSet
...
#         from aiogram_max.methods import GetChatMember
...
#         from aiogram_max.methods import GetChatMemberCount
...
#         from aiogram_max.methods import LeaveChat
...
#         from aiogram_max.methods import UnpinAllChatMessages
...
#         from aiogram_max.methods import UnpinChatMessage
...
#         from aiogram_max.methods import PinChatMessage
...
#         from aiogram_max.methods import SetChatAdministratorCustomTitle
...
#         from aiogram_max.methods import SetChatPermissions
...
#         from aiogram_max.methods import PromoteChatMember
...
#         from aiogram_max.methods import RestrictChatMember
...
#         from aiogram_max.methods import UnbanChatMember
...
#         from aiogram_max.methods import BanChatMember
...
#         from aiogram_max.methods import SetChatDescription
...
#         from aiogram_max.methods import SetChatTitle
...
#         from aiogram_max.methods import DeleteChatPhoto
...
#         from aiogram_max.methods import SetChatPhoto
...
#         from aiogram_max.methods import UnpinAllGeneralForumTopicMessages
...
class Chat(BaseModel):
    ...
    type: ChatType = Field(description="Тип чата: 'chat' — Групповой чат.")
    status: ChatStatus
    title: Optional[str] = Field(
        None,
        description="Отображаемое название чата. Может быть null для диалогов",
    )
    icon: Optional[Image] = Field(None, description="Иконка чата")
    last_event_time: int = Field(description="Время последнего события в чате")
    participants_count: int = Field(
        description="Количество участников чата. Для диалогов всегда 2"
    )
    owner_id: Optional[int] = Field(None, description="ID владельца чата")
    participants: Optional[Any] = Field(
        description="Участники чата с временем последней активности. Может быть null, если запрашивается список чатов"
    )
    is_public: bool = Field(
        description="Доступен ли чат публично (для диалогов всегда false)"
    )
    link: Optional[str] = Field(description="Ссылка на чат")
    description: Optional[str] = Field(description="Описание чата")
    dialog_with_user: Optional[UserWithPhoto] = Field(
        description="Данные о пользователе в диалоге (только для чатов типа 'dialog')"
    )
    messages_count: Optional[int] = Field(
        description="Количество сообщений в чате (доступно только для групповых чатов, недоступно для диалогов)"
    )
    chat_message_id: Optional[str] = Field(
        description="ID сообщения, содержащего кнопку, через которую был инициирован чат"
    )
    pinned_message: Optional[Message] = Field(
