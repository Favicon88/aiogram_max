...
from typing import TYPE_CHECKING, Any, Optional, cast, Union, Literal
...
class UpdateType(BaseModel):
    ...
    update_type: str  # discriminator, но будет конкретизирован в наследниках


class Callback(BaseModel):
    timestamp: int = Field(
        description="Unix-время, когда пользователь нажал кнопку"
    )
...
class MessageCallbackUpdate(UpdateType):
...
class MessageEditedUpdate(UpdateType):
...
class MessageRemovedUpdate(UpdateType):
...
class BotAddedUpdate(UpdateType):
...
class BotRemovedUpdate(UpdateType):
...
class UserAddedUpdate(UpdateType):
...
class UserRemovedUpdate(UpdateType):
...
class BotStartedUpdate(UpdateType):
...
class ChatTitleChangedUpdate(UpdateType):
...
class MessageChatCreatedUpdate(UpdateType):
