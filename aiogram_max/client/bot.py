...
from aiogram_max.utils.token import extract_bot_id, validate_token
...
class Bot:
        ...
        try:
            if isinstance(destination, (str, pathlib.Path)):
                await self.__download_file(
                    destination=destination, stream=stream
                )
                return None
    ...
    async def download(
        ...
        if isinstance(file, str):
            ...
            if file_id is None:
                raise TypeError(
                    "file can only be of the string or Downloadable type"
                )
