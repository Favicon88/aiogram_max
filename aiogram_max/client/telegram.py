...
class SimpleFilesPathWrapper(FilesPathWrapper):
    ...
    def _resolve(
        cls,
        base1: Union[Path, str],
        base2: Union[Path, str],
    ...
    def to_local(self, path: Union[Path, str]) -> Union[Path, str]:
        return self._resolve(
            base1=self.server_path, base2=self.local_path, value=path
        )

    def to_server(self, path: Union[Path, str]) -> Union[Path, str]:
        return self._resolve(
            base1=self.local_path, base2=self.server_path, value=path
        )
...
class TelegramAPIServer:
    ...
    def from_base(cls, base: str, **kwargs: Any) -> "TelegramAPIServer":
        ...
        base = base.rstrip("/")
        return cls(
            base=f"{base}/{{method}}?access_token={{token}}",
            file=f"{base}/{{path}}?access_token={{token}}",
            **kwargs,
        )


PRODUCTION = TelegramAPIServer(
    base="https://botapi.max.ru/{method}?access_token={token}",
    file="https://botapi.max.ru/{path}?access_token={token}",
)
TEST = TelegramAPIServer(
    base="https://botapi.max.ru/bot{token}/test/{method}",
    file="https://botapi.max.ru/file/bot{token}/test/{path}",
)
