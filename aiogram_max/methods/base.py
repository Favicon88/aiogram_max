...
from aiogram_max.client.context_controller import BotContextController
...
class MaxResponse(RootModel[TelegramType]):
    def extract(self) -> TelegramType:
        return self.root


class TelegramMethod(
    BotContextController, BaseModel, Generic[TelegramType], ABC
):
    ...
    def remove_unset(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(values, dict):
            return values
        return {
            k: v for k, v in values.items() if not isinstance(v, UNSET_TYPE)
        }
...
class MaxMethod(TelegramMethod[TelegramType]):
